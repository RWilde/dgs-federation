package com.thesting.user.directive.evaluator;

import com.netflix.graphql.dgs.context.DgsContext;
import com.netflix.graphql.dgs.internal.DgsRequestData;
import com.thesting.user.exception.AccessDeniedError;
 ...
 * 5. Else if security expression evaluates to <code>false</code>,an {@link com.thesting.user.exception.AccessDeniedError} is thrown and resolver function of field is <b>not invoked</b>
 */
@Component
public class SecuredDirectiveWiring implements SchemaDirectiveWiring {
    ...
    public GraphQLObjectType onObject(SchemaDirectiveWiringEnvironment<GraphQLObjectType> environment) {
        ...
        StringValue expressionValue = (StringValue) field.getDirective(SECURED_DIRECTIVE).getArgument(REQUIRES_ATTR).getArgumentValue().getValue();
        ...
        for (GraphQLFieldDefinition fieldDefinition : field.getFieldDefinitions()) {
            ...
            DataFetcher authDataFetcher = dataFetchingEnvironment -> {
                ...
                String userUuid = requestData.getHeaders().getFirst("token");

                DataFetcherResult.Builder<Object> resultBuilder = DataFetcherResult.newResult();
                boolean result = directiveEvaluator.evaluateExpression(expressionValue.getValue(), userUuid, fieldDefinition.getName());
    ...
    public GraphQLFieldDefinition onField(SchemaDirectiveWiringEnvironment<GraphQLFieldDefinition> environment) {
        ...
        DataFetcher authDataFetcher = dataFetchingEnvironment -> {
            ...
            StringValue expressionValue = (StringValue) dataFetchingEnvironment.getFieldDefinition().getDirective(SECURED_DIRECTIVE).getArgument(REQUIRES_ATTR).getArgumentValue().getValue();
            DataFetcherResult.Builder<Object> resultBuilder = DataFetcherResult.newResult();
            boolean result = directiveEvaluator.evaluateExpression(expressionValue.getValue(), userUuid, field.getName());
